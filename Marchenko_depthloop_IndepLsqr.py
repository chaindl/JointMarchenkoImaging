...
def Marchenko_depthloop_indepLsqr(zinvs,zendvs):
    ...
    for iz in range(nvsz):
        ...
        for ix in range(nvsx):
            ...
            f1_1_minus, f1_1_plus, g_1_minus, g_1_plus = MF.machenkoFocSourceLoop_Lsqr(direct,toff,g0VS,iava1,Rop1,R1op1,Restrop1,t)
            f1_2_minus, f1_2_plus, g_2_minus, g_2_plus = MF.machenkoFocSourceLoop_Lsqr(direct,toff,g0VS,iava2,Rop2,R1op2,Restrop2,t)
        ...
        np.savetxt('datasets_indep/Line1_' + str(vsz[iz]) + '.dat', np.diag(redatumed1[:,:,(nt+1)/jt-1]), delimiter=',')
        np.savetxt('datasets_indep/Line2_' + str(vsz[iz]) + '.dat', np.diag(redatumed2[:,:,(nt+1)/jt-1]), delimiter=',')
        
        I1[:,iz]=np.diag(redatumed1[:,:,(nt+1)/jt-1])
        I2[:,iz]=np.diag(redatumed2[:,:,(nt+1)/jt-1])
     
    np.savetxt('datasets_indep/Image1' + str(zinvs) + '-' + str(zendvs) + '.dat', I1, delimiter=',')
    np.savetxt('datasets_indep/Image2' + str(zinvs) + '-' + str(zendvs) + '.dat', I2, delimiter=',')
